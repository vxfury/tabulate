...
void check_reset()
{
    ...
    dual_println("Checking that after reset() the thread pool reports a number of threads equal to half the hardware "
                 "concurrency...");
    check(pool.get_worker_size() == std::thread::hardware_concurrency() / 2);
...
{
    ui32 n = pool.get_worker_size() * 10;
...
/**
 * @brief Check that parallelize() works for a specific number of indices split over a specific number of tasks.
 *
...
{
    if (start == end) end++;
    dual_println("Verifying that a loop from ", start, " to ", end, " with ", num_tasks,
                 num_tasks == 1 ? " task" : " tasks", " modifies all indices...");
    ...
    pool.parallelize(
        ...
        [&flags, &offset](const i32 &start, const i32 &end) {
            for (i32 i = start; i < end; i++) flags[(ui64)(i - offset)] = true;
        },
        num_tasks);
    bool all_flags = true;
    for (ui64 i = 0; i < num_indices; i++) all_flags = all_flags && flags[i];
    check(all_flags);
}

/**
 * @brief Check that parallelize() works using several different random values for the range of indices and number
 * of tasks.
 */
void check_parallelize_loop()
{
    std::mt19937_64 mt(rd());
    std::uniform_int_distribution<i32> index_dist((i32)pool.get_worker_size() * -100,
                                                  (i32)pool.get_worker_size() * 100);
    std::uniform_int_distribution<ui32> task_dist(1, pool.get_worker_size());
    for (ui32 i = 0; i < 10; i++) check_parallelize_loop(index_dist(mt), index_dist(mt), task_dist(mt));
}
...
void check_task_monitoring()
{
    ...
    dual_println("After submission, should have: ", n * 3, " tasks total, ", n, " tasks running, ", n * 2,
                 " tasks queued...");
    check(pool.get_task_size_unfinished() == n * 3 && pool.get_task_size_running() == n
          && pool.get_task_size_queued() == n * 2);
    ...
    check(pool.get_task_size_unfinished() == n * 2 && pool.get_task_size_running() == n
          && pool.get_task_size_queued() == n);
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    dual_println("After releasing ", n, " more tasks, should have: ", n, " tasks total, ", n, " tasks running, ", 0,
                 " tasks queued...");
    check(pool.get_task_size_unfinished() == n && pool.get_task_size_running() == n
          && pool.get_task_size_queued() == 0);
    ...
    dual_println("After releasing the final ", n, " tasks, should have: ", 0, " tasks total, ", 0, " tasks running, ",
                 0, " tasks queued...");
    check(pool.get_task_size_unfinished() == 0 && pool.get_task_size_running() == 0
          && pool.get_task_size_queued() == 0);
...
void check_pausing()
{
    ...
    check(pool.get_task_size_unfinished() == n && pool.get_task_size_running() == 0
          && pool.get_task_size_queued() == n);
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
...
/**
 * @brief A lightweight matrix class template for performance testing purposes. Not for general use; only contains the
 * bare minimum functionality needed for the test. Based on https://github.com/bshoshany/multithreaded-matrix
 *
...
class matrix {
  public:
     ...
     */
    matrix(const ui64 &_rows, const ui64 &_cols) : rows(_rows), cols(_cols), smart_elements(new T[rows * cols])
    {
     ...
     */
    matrix(const matrix<T> &m) : rows(m.rows), cols(m.cols), smart_elements(new T[rows * cols])
    {
        elements = smart_elements.get();
        for (ui64 i = 0; i < rows * cols; i++) elements[i] = m.elements[i];
    }
     ...
     */
    matrix(matrix<T> &&m) : rows(m.rows), cols(m.cols), smart_elements(std::move(m.smart_elements))
    {
        elements = smart_elements.get();
        m.rows = 0;
        m.cols = 0;
    ...
    bool operator==(const matrix<T> &m) const
    {
        ...
        for (ui64 i = 0; i < rows * cols; i++) compare_result = compare_result && (elements[i] == m.elements[i]);
    ...
    matrix<T> transpose(const ui32 &num_tasks) const
    {
        ...
        if (num_tasks == 0) {
            for (ui64 i = 0; i < out.rows; i++)
                for (ui64 j = 0; j < out.cols; j++) out(i, j) = operator()(j, i);
        } else {
            pool.parallelize(
                0, out.rows,
    ...
    friend matrix<T> add_matrices(const matrix<T> &a, const matrix<T> &b, const ui32 &num_tasks)
    {
        ...
        if (num_tasks == 0)
            for (ui64 i = 0; i < a.rows * a.cols; i++) c[i] = a[i] + b[i];
        else {
            pool.parallelize(
                0, a.rows * a.cols,
                [&a, &b, &c](const ui64 &start, const ui64 &end) {
                    for (ui64 i = start; i < end; i++) c[i] = a[i] + b[i];
                },
                num_tasks);
        }
        return c;
        ...
        if (num_tasks == 0) {
            for (ui64 i = 0; i < a.rows; i++)
                for (ui64 j = 0; j < b.cols; j++) {
                    c(i, j) = 0;
                    for (ui64 k = 0; k < a.cols; k++) c(i, j) += a(i, k) * b(k, j);
                }
        } else {
            pool.parallelize(
                0, a.rows,
                [&a, &b, &c, &a_cols = a.cols, &b_cols = b.cols](const ui64 &start, const ui64 &end) {
                    for (ui64 i = start; i < end; i++)
                        for (ui64 j = 0; j < b_cols; j++) {
                            c(i, j) = 0;
                            for (ui64 k = 0; k < a_cols; k++) c(i, j) += a(i, k) * b(k, j);
                        }
                },
                num_tasks);
        }
...
class random_matrix_generator {
  public:
     ...
     * @param params The parameters to pass to the constructor of the distribution. The number of parameters and their
     * types depends on the particular distribution being used.
     */
    template <typename... P>
    ...
    matrix<T> generate_matrix(const ui64 &rows, const ui64 &cols, const ui32 &num_tasks)
    {
        ...
        if (num_tasks == 0) {
            ...
            for (ui64 i = 0; i < rows * cols; i++) m[i] = dist(mt);
        } else {
            pool.parallelize(
                0, rows * cols,
                [this, &m](const ui64 &start, const ui64 &end) {
                    std::mt19937_64 mt(generate_seed());
                    for (ui64 i = start; i < end; i++) m[i] = dist(mt);
                },
                num_tasks);
        }
        return m;
  ...
  private:
    ...
    /**
     * @brief Generate a seed. The std::mt19937_64 in each block will be seeded using this function in order to avoid
     * depleting the entropy of the random_device.
     *
...
/**
 * @brief Check the matrix class template by comparing the results of adding, multiplying, and transposing matrices
 * calculated in two ways: single-threaded and multithreaded.
 */
void check_matrix()
{
    // Initialize a random_matrix_generator object to generates matrices with integers uniformly distributed between
    // -1000 and 1000.
    random_matrix_generator<i64, std::uniform_int_distribution<i64>> rnd(-1000, 1000);
    // Define the size of the matrices to use.
    const ui32 thread_count = pool.get_worker_size();
...
void check_performance()
{
    ...
    // Initialize a random_matrix_generator object to generates matrices with real (floating-point) numbers uniformly
    // distributed between -1000 and 1000.
    ...
    // If the CPU has more than 8 threads, we leave 2 threads for the rest of the operating system. Otherwise,
    // performance may suffer.
    const ui32 thread_count = pool.get_worker_size() <= 8 ? pool.get_worker_size() : pool.get_worker_size() - 2;
    dual_println("Using ", thread_count, " out of ", pool.get_worker_size(), " threads.");
    ...
    // Generate two random test matrices to be used for benchmarking addition, transposition, and random matrix
    // generation.
    matrix<double> A = rnd.generate_matrix(rows, cols, thread_count);
    matrix<double> B = rnd.generate_matrix(rows, cols, thread_count);

    // Generate two random test matrices to be used for benchmarking multiplication. Since matrix multiplication is
    // O(n^3), we reduce the size of the test matrices so that this operation completes within a reasonable time.
...
int main()
{
    ...
    print_header("Checking that parallelize() works:");
    ...
    if (tests_failed == 0) {
        ...
        print_header("FAILURE: Passed " + std::to_string(tests_succeeded) + " checks, but failed "
                     ...
                     '+');
        dual_println("\nPlease submit a bug report at https://github.com/bshoshany/thread-pool/issues including the "
